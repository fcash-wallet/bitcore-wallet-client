...
PayPro.get = function(opts, cb) {
  ...
  var fcash = Fcash_[coin];

  var COIN = coin.toUpperCase();
  var PP = new FcashPayPro(COIN);

  opts.headers = opts.headers || {
    'Accept': FcashPayPro.LEGACY_PAYMENT[COIN].REQUEST_CONTENT_TYPE,
  ...
  http(opts, function(err, dataBuffer) {
    ...
    try {
      var body = FcashPayPro.PaymentRequest.decode(dataBuffer);
    ...
    var decodedDetails = FcashPayPro.PaymentDetails.decode(serializedDetails);
    var pd = new FcashPayPro();
    ...
    var addr = new fcash.Address.fromScript(new fcash.Script(scriptBuf), network);
...
PayPro._getPayProRefundOutputs = function(addrStr, amount, coin) {
  ...
  var fcash = Fcash_[coin];
  var output = new FcashPayPro.Output();
  var addr = new fcash.Address(addrStr);

  var s;
  if (addr.isPayToPublicKeyHash()) {
    s = fcash.Script.buildPublicKeyHashOut(addr);
  } else if (addr.isPayToScriptHash()) {
    s = fcash.Script.buildScriptHashOut(addr);
  } else {
...
PayPro.send = function(opts, cb) {
  ...
  opts.method = 'POST';
  opts.headers = opts.headers || {
    'Accept': FcashPayPro.LEGACY_PAYMENT[COIN].ACK_CONTENT_TYPE,
    'Content-Type': FcashPayPro.LEGACY_PAYMENT[COIN].CONTENT_TYPE,
    // 'Content-Type': 'application/octet-stream',
  };
  opts.body = payment;
  ...
  http(opts, function(err, rawData) {
    ...
    if (rawData) {
      try {
        var data = FcashPayPro.PaymentACK.decode(rawData);
        var pp = new FcashPayPro(COIN);
