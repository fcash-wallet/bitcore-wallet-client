...
Credentials.create = function(coin, network) {
  ...
  x.coin = coin;
  x.network = network;
  x.xPrivKey = (new Fcash.HDPrivateKey(network)).toString();
...
Credentials.fromExtendedPublicKey = function(coin, xPubKey, source, entropySourceHex, account, derivationStrategy, opts) {
  ...
  x.coin = coin;
  x.xPubKey = xPubKey;
  x.entropySource = Fcash.crypto.Hash.sha256sha256(entropyBuffer).toString('hex');
  x.account = account;
  x.derivationStrategy = derivationStrategy;
...
Credentials.prototype._hashFromEntropy = function(prefix, length) {
  ...
  var b2 = Fcash.crypto.Hash.sha256hmac(b, new Buffer(prefix));
...
Credentials.prototype._expand = function() {
  ...
  if (this.xPrivKey) {
    var xPrivKey = new Fcash.HDPrivateKey.fromString(this.xPrivKey);
  ...
  if (this.entropySourcePath) {
    ...
    this.entropySource = Fcash.crypto.Hash.sha256sha256(seed).toString('hex');
  }

  if (this.entropySource) {
    // request keys from entropy (hw wallets)
    var seed = this._hashFromEntropy('reqPrivKey', 32);
    var privKey = new Fcash.PrivateKey(seed.toString('hex'), network);
    ...
    this.entropySource = Fcash.crypto.Hash.sha256(requestDerivation.privateKey.toBuffer()).toString('hex');
  }
...
Credentials.prototype.getDerivedXPrivKey = function(password) {
  ...
  var xPrivKey = new Fcash.HDPrivateKey(this.getKeys(password).xPrivKey, this.network);
  var deriveFn = !!this.compliantDerivation ? _.bind(xPrivKey.deriveChild, xPrivKey) : _.bind(xPrivKey.deriveNonCompliantChild, xPrivKey);
...
Credentials.fromOldCopayWallet = function(w) {
  function walletPrivKeyFromOldCopayWallet(w) {
    ...
    var privKey = new Fcash.PrivateKey.fromBuffer(Fcash.crypto.Hash.sha256(seedBuf));
  ...
  var pkr = _.map(w.publicKeyRing.copayersExtPubKeys, function(xPubStr) {
    ...
    if (isMe) {
      ...
      requestDerivation = (new Fcash.HDPrivateKey(credentials.xPrivKey))
        .deriveChild(path).hdPublicKey;
    } else {
      // this 
      var path = Constants.PATHS.REQUEST_KEY_AUTH;
      requestDerivation = (new Fcash.HDPublicKey(xPubStr)).deriveChild(path);
    }

    // Grab Copayer Name
    var hd = new Fcash.HDPublicKey(xPubStr).deriveChild('m/2147483646/0/0');
