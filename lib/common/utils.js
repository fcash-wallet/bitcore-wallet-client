...
Utils.privateKeyToAESKey = function(privKey) {
  ...
  $.checkArgument(Fcash.PrivateKey.isValid(privKey), 'The private key received is invalid');
  var pk = Fcash.PrivateKey.fromString(privKey);
  return Fcash.crypto.Hash.sha256(pk.toBuffer()).slice(0, 16).toString('base64');
};
...
Utils.deriveAddress = function(scriptType, publicKeyRing, path, m, network, coin) {
  ...
  var publicKeys = _.map(publicKeyRing, function(item) {
    var xpub = new fcash.HDPublicKey(item.xPubKey);
  ...
  switch (scriptType) {
    case Constants.SCRIPT_TYPES.P2SH:
      fcashAddress = fcash.Address.createMultisig(publicKeys, m, network);
      break;
    case Constants.SCRIPT_TYPES.P2PKH:
      $.checkState(_.isArray(publicKeys) && publicKeys.length == 1);
      fcashAddress = fcash.Address.fromPublicKey(publicKeys[0], network);
      break;
  }
...
Utils.buildTx = function(txp) {
  ...
  if (txp.toAddress && txp.amount && !txp.outputs) {
    ...
    _.each(txp.outputs, function(o) {
      ...
      if (o.script) {
        t.addOutput(new fcash.Transaction.Output({
          script: o.script,
          satoshis: o.amount
        }));
      } else {
  ...
  // Validate inputs vs outputs independently of Fcash
