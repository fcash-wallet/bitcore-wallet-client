...
API.prototype.validateKeyDerivation = function(opts, cb) {
  ...
  function testHardcodedKeys() {
    ...
    var xpub = Fcash.HDPublicKey.fromString('xpub6BosfCnifzxcFwrSzQiqu2DBVTshkCXacvNsWGYJVVhhawA7d4R5WSWGFNbi8Aw6ZRc1brxMyWMzG3DSSSSoekkudhUd9yLb6qx39T9nMdj');
  ...
  function testLiveKeys() {
    ...
    if (!xpriv) {
      xpriv = new Fcash.HDPrivateKey(c.xPrivKey);
    }
    xpriv = xpriv.deriveChild(c.getBaseAddressDerivationPath());
    var xpub = new Fcash.HDPublicKey(c.xPubKey);
...
API.prototype.decryptBIP38PrivateKey = function(encryptedPrivateKeyBase58, passphrase, opts, cb) {
  ...
  var privateKey = new Fcash.PrivateKey(privateKeyWif);
  var address = privateKey.publicKey.toAddress().toString();
  var addrBuff = new Buffer(address, 'ascii');
  var actualChecksum = Fcash.crypto.Hash.sha256sha256(addrBuff).toString('hex').substring(0, 8);
  var expectedChecksum = Fcash.encoding.Base58Check.decode(encryptedPrivateKeyBase58).toString('hex').substring(6, 14);
...
API._buildSecret = function(walletId, walletPrivKey, coin, network) {
  if (_.isString(walletPrivKey)) {
    walletPrivKey = Fcash.PrivateKey.fromString(walletPrivKey);
  }
  var widHex = new Buffer(walletId.replace(/-/g, ''), 'hex');
  var widBase58 = new Fcash.encoding.Base58(widHex).toString();
...
API.parseSecret = function(secret) {
  ...
  try {
    ...
    var widHex = Fcash.encoding.Base58.decode(widBase58).toString('hex');
    var walletId = split(widHex, [8, 12, 16, 20]).join('-');

    var walletPrivKey = Fcash.PrivateKey.fromString(secretSplit[1]);
...
API.signTxp = function(txp, derivedXPrivKey) {
  ...
  var xpriv = new Fcash.HDPrivateKey(derivedXPrivKey);
...
API.prototype._addSignaturesToFcashTx = function(txp, t, signatures, xpub) {
  if (signatures.length != txp.inputs.length)
    throw new Error('Number of signatures does not match number of inputs');

  $.checkState(txp.coin);

  var fcash = Fcash_[txp.coin];
...
API.prototype._applyAllSignatures = function(txp, t) {
  ...
  _.each(sigs, function(x) {
    self._addSignaturesToFcashTx(txp, t, x.signatures, x.xpub);
  });
};
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  ...
  var walletPrivKey = opts.walletPrivKey || new Fcash.PrivateKey();
  ...
  var args = {
    ...
    m: m,
    n: n,
    pubKey: (new Fcash.PrivateKey(walletPrivKey)).toPublicKey().toString(),
    coin: coin,
    network: network,
...
API.prototype.recreateWallet = function(cb) {
  ...
  this.getStatus({
    ...
    var c = self.credentials;
    var walletPrivKey = Fcash.PrivateKey.fromString(c.walletPrivKey);
...
API.prototype.addAccess = function(opts, cb) {
  ...
  var reqPrivKey = new Fcash.PrivateKey(opts.generateNewKey ? null : this.credentials.requestPrivKey);
  var requestPubKey = reqPrivKey.toPublicKey().toString();

  var xPriv = new Fcash.HDPrivateKey(this.credentials.xPrivKey)
